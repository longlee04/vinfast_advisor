"""Run the multi-turn slot evaluation and emit Markdown or JSON."""
...
from eval.conversation.loader import (  # noqa: E402
    DEFAULT_DATASET_PATH,
    load_conversation_dataset,
)
from eval.conversation.metrics import calculate_metrics  # noqa: E402
from eval.conversation.models import ConversationDataset, ConversationRun  # noqa: E402
from eval.conversation.reporting import render_json, render_markdown  # noqa: E402
from scripts.slot_conversation_runner import run_conversation_dataset  # noqa: E402

LIVE_ENABLE_VARIABLE = "AGENT_SLOT_EVAL_LIVE"
...
def select_scenarios(dataset: ConversationDataset, scenario_ids: list[str] | None) -> ConversationDataset:
...
async def _main() -> int:
    ...
    try:
        dataset = select_scenarios(load_conversation_dataset(args.dataset), args.scenario_ids)
    except ValueError as error:
        raise SystemExit(str(error)) from error
    scenarios = []
    for _ in range(args.repeat):
        current = await run_conversation_dataset(dataset, live_llm=live_llm)
        scenarios.extend(current.scenarios)
    run = ConversationRun(scenarios=scenarios)
