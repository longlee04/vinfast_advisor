...
> **Ghi chú bản cá nhân:** hệ thống AI Usage Logging của lớp học (thư mục `.ai-log/`,
> `scripts/setup_hooks.sh`, `scripts/log_hook.py`, `scripts/submit_log.py` và các biến
> `AI_LOG_*`) đã được gỡ khỏi repo này. Phần hướng dẫn liên quan bên dưới giữ lại để
> tham khảo, không còn chạy được.
