...
| 4 | Migration 0037/0038 + view, domain thuần (stage, heat, R0–R7, buyer_for, insight, overview), job nền + hook sau `commit_core_turn`, extractor/phân loại LLM, API hồ sơ ≤ 3 SQL, cơ hội theo độ nóng, dashboard số thật (bỏ số bịa ở cả BE lẫn FE), picker phân công, sweep/backfill script, đồng bộ tên/SĐT từ `auth_user_profiles` | **4G (hỏi lồng thời điểm mua) CHƯA LÀM** — chạm `core/act.py` đang có thay đổi dở của người dùng. Câu MỞ phiên không tính là tín hiệu restart (R4) — phát hiện khi viết test |
